...
if love.keyboard.wasPressed('return') then
	...
	for i = 3, self.scoreIndex, -1 do
		self.highScores[i + 1] = {
		name = self.highScores[i].name,
		score = self.highScores[i].record
		}
	end
	
	self.highScores[self.scoreIndex].name = name
	self.highScores[self.scoreIndex].record = self.score
	
	local scoresStr = ''
	
	for i =1, 3 do
		scoresStr = scoresStr .. self.highScores[i].name .. '\n'
		scoresStr = scoresStr .. tostring(self.highScores[i].record) .. '\n'
	end
	
	love.filesystem.write('breakout/breakout.txt', scoresStr)
